...
### 11.0 当前验收基线（2026-08-10）

平台已完成有效令牌和四项负向探测。两个服务当前结果一致：

| 检查 | 客户画像 | 产品考点 |
|---|---|---|
| 有效 Token | 通过 | 通过 |
| 无 Token 必须拒绝 | 未通过 | 未通过 |
| 错误 audience 必须拒绝 | 未通过 | 未通过 |
| 缺 scope 必须拒绝 | 未通过 | 未通过 |
| 错误 tool binding 必须拒绝 | 未通过 | 未通过 |

这说明服务目前能够接收 EA Token，但尚未强制校验。平台已将两项服务保持在 `shadow`，开发者完成以下整改并通过同一组自动探测后，才能切换为 `enforced`。
