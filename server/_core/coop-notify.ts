 ...
 *   (1) 被通知人的飞书（通过 JiuwenSwarm 飞书 Bot 双向绑定）
 *   (2) 未来：浏览器 toast / 左侧红点（WS 广播）
 *
 * 当前实现：飞书私聊推送；WS 广播交给后续实时化
 */
  ...
  try {
    switch (ev.type) {
      case "session_created": {
        // 给每个被邀请人发飞书。通知是 best-effort，不影响协作创建。
        ...
        for (const m of ev.members) {
          ...
          if (!targetAdoptId) {
            console.log(`[coop-notify] user ${m.userId} has no active adoption, skip feishu`);
            continue;
          }
          const msg = `【协作邀请】${ev.creatorName} 邀请你参与「${ev.title}」\n分配给你的任务：${m.subtask}\n电脑打开岗位智能体，或手机长按链接「在浏览器中打开」（微信内登录态可能不稳）：\n${sessionUrl}`;
      ...
      case "member_rejected": {
        // 暂不向发起人推送（避免自己给自己刷屏）；
        // 后续可按 creatorUserId 查 adoptId 再推送
