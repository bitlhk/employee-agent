...
describe("ClaudeCodeProvider", () => {
  ...
  it("uses OpenAI-compatible chat completions when the legacy resolver marks the binding", async () => {
    ...
    const result = await new ClaudeCodeProvider(provider({ runtimeFamily: "claude-code", authType: "bearer-token" }), fetchImpl as any).dispatch({
    ...
    expect(seenHeaders["x-openclaw-session-key"]).toBe("cluster-run-1");
    expect(seenHeaders["x-tenant-token"]).toBeUndefined();
    expect(seenHeaders["x-tenant-workspace"]).toBeUndefined();
