...
个人知识库支持 PDF、Office、Markdown、文本和结构化文件，上传后自动解析并建立本地检索索引。企业知识与当前岗位知识按相关度自动检索，个人知识可由用户显式选择；回答会标注实际文件与位置。自动检索使用相关度门控和上下文预算，未命中时不会增加主模型上下文。未配置 embedding 时使用 LlamaIndex + BM25，配置兼容的 embedding 服务后自动启用 FAISS + RRF 混合检索。
