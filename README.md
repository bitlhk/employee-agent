...
智能体可以记住用户明确确认的输出偏好和稳定工作习惯；自动发现的内容需要在不同会话中重复出现后才会生效。智能体记忆按“协作依据、学习中、已沉淀”三层展示，并按岗位实例隔离，支持确认、修正、忘记和关闭。

**可核验的知识中心**

个人知识库支持 PDF、Office、Markdown、文本和结构化文件，上传后按章节、页码和表格结构建立父子索引。企业知识与当前岗位知识按相关度自动检索，个人知识可由用户显式选择；回答会标注文档版本、章节和页码。自动检索使用相关度门控和上下文预算，未命中时不会增加主模型上下文。未配置 embedding 时使用 LlamaIndex + BM25，配置兼容的 embedding 服务后启用 FAISS + RRF 混合检索，并可选接入 Reranker；外部处理被禁止或组件故障时会自动降级。使用远程 Embedding 或 Reranker 会向相应服务发送必要的文档片段或查询，敏感知识应关闭外部处理并使用本地模型。
...
内置不依赖模型的确定性数据护栏：高置信识别凭据、Token、私钥、手机号、身份证和银行卡信息；凭据与私钥在 EA 管理的外发通道中直接阻断，个人信息自动脱敏，托管记忆、诊断日志和审计元数据同步保护。处置事件进入审计账本，并支持 `enforce`、`monitor`、`off` 三种部署模式。

提供 `/admin` 管理入口，用于用户、智能体、技能、工具、系统状态和审计类能力的管理。适合在企业环境中做权限分配、问题排查、阻断事件追踪和使用情况核验。
...
- 数据护栏覆盖 EA 管理的日志、托管记忆和通知外发；Runtime 工具直接向第三方 API 发送的数据，仍需结合工具授权、沙箱网络策略和上游系统权限控制。
...
```text
/              -> 首页：登录、申请岗位智能体、进入工作台
/claw/:adoptId -> 岗位智能体工作台：对话、技能、记忆、知识、协作、工作区、定时任务
...
bash /tmp/employee-agent-install.sh \
  ...
  --ref 3f4072fe312644e62a1396cbd3b0e003b0e7ce97 \
  --expected-commit 3f4072fe312644e62a1396cbd3b0e003b0e7ce97 \
  --mirror auto \
